        ...
        for j, cell in enumerate(papers[i]):
            if cell == 1:
                ...
                if upper_square_row + middle_square_row + lower_square_row < 4:
                    indexes_to_update.append({'i':i,'j':j})
                    counter += 1
    for entry_to_update in indexes_to_update:
        papers[entry_to_update['i']][entry_to_update['j']] = 0
    return counter
...
counter = 0
removed_paper = -1
while removed_paper != 0:
    removed_paper = remove_papers()
    counter += removed_paper
print(counter)